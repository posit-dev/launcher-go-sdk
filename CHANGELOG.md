...
- **New `launcher.SettingsReloadablePlugin` interface**: implement `SettingsReloader() *settings.Reloader` to have the SDK automatically resolve, apply, and report the dual-homed settings on every config reload. This is one of **two** reload-related interfaces now, and they are not interchangeable: `ConfigReloadablePlugin.ReloadConfig` is a plain hook for your own reload work (profiles, etc.) with no dual-homed-settings involvement; `SettingsReloadablePlugin` takes over the whole config-reload response (dual-homed settings *and* your own reload work, via the returned `Reloader`'s `ApplyExtra` hook) and takes precedence when a plugin implements both — a plain `ReloadConfig` is never called in that case.
